""
XAI Integration Module for PhishGuard AI
Provides explainable AI functionality for phishing detection models.
"""
import os
import json
import joblib
...
class XAIHandler:
    ...
    def generate_explanation_report(self, model_type: str, input_data: Union[str, Dict], 
        ...
        if model_type == 'url' and isinstance(input_data, str):
            ...
            for keyword in suspicious_keywords:
                if keyword in url:
                    explanations.append({
                        'type': 'warning',
                        'message': f'The URL contains "{keyword}" which is commonly used in phishing attempts.',
                        'source': 'rule_based',
            ...
            if input_data.get('spf_fail', False):
                explanations.append({
                    'type': 'warning',
                    'message': 'SPF check failed. The sending server is not authorized to send emails for this domain.',
                    'source': 'rule_based',
                    'feature': 'spf_fail'
                })
            ...
            if input_data.get('dkim_fail', False):
                explanations.append({
                    'type': 'warning',
                    'message': 'DKIM signature verification failed. The email may have been tampered with.',
                    'source': 'rule_based',
                    'feature': 'dkim_fail'
                })
            ...
            if input_data.get('dmarc_fail', False):
                explanations.append({
                    'type': 'warning',
                    'message': 'DMARC check failed. The email does not comply with the domain\'s email authentication policy.',
                    'source': 'rule_based',
                    'feature': 'dmarc_fail'
                })
    ...
    def format_explanations_for_ui(self, explanation_report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if shap_exp and 'feature_importance' in shap_exp:
            ...
            formatted['explanations'].append({
                'type': 'info',
                'message': f"Key features influencing this prediction: {', '.join(top_features)}",
                'source': 'shap'
            })
    ...
    def generate_xai_report(self, analysis_type: str, analysis_data: Dict[str, Any], 
                          raw_input: str) -> str:
        """
    ...
    def download_explanation_report(self, analysis_type: str, analysis_data: Dict[str, Any], 
                                 raw_input: str, filename: str = None) -> str:
        """
        ...
        if not filename:
            ...
            filename = f"phishguard_report_{analysis_type}_{timestamp}.txt"
